            ...
            <div class="bg-white p-8 rounded-3xl shadow-lg border border-[#D4AF37]/20">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="precompra" value="{{ old('precompra') }}" class="w-full bg-[#FDF2D0]/30 border-[#D4AF37]/30 rounded-xl p-3" placeholder="0.00">
                    </div>

                    {{-- Preventa (Precio) --}}
                    <div>
                        <label class="block text-sm font-bold mb-2 uppercase tracking-wide">Precio (Preventa)</label>
                        <input type="number" step="0.01" name="preventa" value="{{ old('preventa') }}" class="w-full bg-[#FDF2D0]/30 border-[#D4AF37]/30 rounded-xl p-3" placeholder="0.00">
                    </div>

                    {{-- Inventario (Stock) --}}
                    <div>
                        <label class="block text-sm font-bold mb-2 uppercase tracking-wide">Stock Inicial (Inventario)</label>
                        <input type="number" name="inventario" value="{{ old('inventario') }}" class="w-full bg-[#FDF2D0]/30 border-[#D4AF37]/30 rounded-xl p-3" placeholder="0">
                    </div>
